...
# -*- coding: utf-8 -*-
'''
Running module for PostgreSQL testing
'''
...
def build_database(fieldnames,datalist):
    ...
    insert_query = insert_query.format(l=full_list)
...
def main():
    ...
    # command line
    # python cprefsql_postgre.py -d detail.csv -i input.csv -r pref.txt -a partition -t -1
    ...
    if topk <0 :
        ...
        if algorithm == TUP_ALG_BNL_STAR_STAR:
            basic_query = basic_query.format(func="best_bnl", pref = pref_text)
        elif algorithm == TUP_ALG_PARTITION:
            basic_query = basic_query.format(func="best_partition", pref = pref_text)
        elif algorithm == TUP_ALG_MAX_PREF:
            basic_query = basic_query.format(func="best_maxpref", pref = pref_text)
        ...
        if algorithm == TUP_ALG_BNL_STAR_STAR:
            basic_query = basic_query.format(func="topk_bnl", pref = pref_text, topk = topk)
        elif algorithm == TUP_ALG_PARTITION:
            basic_query = basic_query.format(func="topk_partition", pref = pref_text, topk = topk)
        elif algorithm == TUP_ALG_MAX_PREF:
            basic_query = basic_query.format(func="topk_maxpref", pref = pref_text, topk = topk)
    ...
    print("Number of best tupples: ", len(rec_list))
